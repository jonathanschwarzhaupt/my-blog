...
| `development` | Persistent integration branch; a `development`->`main` PR being opened or updated triggers the dev-image workflow (once per completed feature, not once per issue — see `docs/adr/0008-tag-based-dev-image-versioning-on-pr-to-main.md`) | **Rebase-merge** into `main` (never squashed, and — see the note below — never a true two-parent merge commit either) | release-please derives one changelog entry per conventional-commit-formatted commit on the branch it watches. Read directly from release-please's own source (`src/commit.ts`'s `splitMessages()`), GitHub's default squash-merge commit body (`* ` bullets, single newlines) does not get split back into multiple entries by that parser, so squashing `development → main` would collapse an entire batch of features into a single undifferentiated changelog line. Rebase-merging replays each feature's own commit individually onto `main` (new SHAs, but each still its own conventional-commit-formatted message), so `main` gets one changelog entry per feature — the reason this level's strategy is deliberately different from the level below it. |
| `main` | release-please's target; a merge here is a real release event | — | Tags and GitHub Releases are only ever cut from here. |

**Why not a true (two-parent) merge commit.** This was the original guidance here, and it silently breaks release-please: its manifest releaser walks `main`'s commit history newest-first *by commit date* and stops as soon as it reaches the previous release's commit SHA (`src/manifest.ts`, the `releaseCommitsFound >= expectedShas` break). A true merge preserves each incoming commit's original date — so if `development` sat unmerged for a while, its commits can be chronologically *older* than `main`'s own last release commit (release-please's own auto-generated `chore(main): release X.Y.Z` commit, created whenever the previous Release PR was merged). The walk hits that release commit and stops before ever reaching the older-but-logically-new commits from `development`, so they get silently excluded from the next changelog and version bump — permanently, since no later commit changes their dates. This was discovered empirically (see the commit graph around `70a4d97`/`b06443b` in the repo's history: `#48`/`#49`/`#52`/`#53`'s commits predate `b06443b` and were never picked up). Rebase-merging avoids this because rebasing rewrites each commit's date to "now," keeping them newer than any prior release commit in the walk.
...
- **Dev channel** (`dev-image.yml`, triggered when a `development`->`main` PR is opened or gets a new push while open): computes the next version via `release-please release-pr --dry-run` against `development`'s HEAD — read-only against GitHub's API, creates no PR — and tags the image `X.Y.Z-dev.<run_number>.<short-sha>` (a valid SemVer 2.0 prerelease identifier, and a valid Docker tag, since Docker tags can't contain `+`) plus a floating `:dev`. It also pushes a real git tag, `vX.Y.Z-dev.<run_number>.<short-sha>`, at the exact commit built, so `internal/vcs.Version()` reports that string instead of a bare commit SHA; this is safe against `main`'s own tag list and release-please's bookkeeping there specifically because `development`->`main` is a rebase-merge (see the note above) — the tagged commit's SHA can never become reachable from `main`, so release-please can never mistake it for a real release (see `docs/adr/0008-tag-based-dev-image-versioning-on-pr-to-main.md` for the full argument).
