...
**Scope boundary.** This repo's responsibility ends at a correctly-tagged, correctly-versioned image landing in GHCR (or, for the rc channel, a floating tag) and a database branch existing with its DSN recorded somewhere consumable. The Kubernetes manifests, GitOps policies (Flux `ImagePolicy` etc.), and Renovate config that actually deploy those images live in the separate homelab infrastructure repo, not here.

### Branching model

**`main` is the sole permanent branch.** release-please watches it directly — this is release-please's own native model (one release branch, one rolling Release PR against it; see `docs/adr/0010-retire-development-for-trunk-based-release-please.md` for the full reasoning, including primary-source citations from release-please's own design docs and why a persistent second branch fights the tool rather than working with it). There is **no persistent `development` branch** — one existed previously, was found to permanently diverge from `main` in a confusing way, and was retired. If you see references to `development` in older docs, PRs, or ADRs (`0006`–`0009`), they describe the prior model; ADR-0010 is the current source of truth.

| Branch | Role | Merge strategy in | Why |
|---|---|---|---|
| `feature/*` | One per issue, branched off `main` **or** off a `story/*` branch if the issue is part of a larger multi-issue story/PRD | Squash-merged into whichever it's branched off | A feature branch typically accumulates messy in-progress commits (fixups, review-response commits); squashing collapses that into one clean commit before it ever reaches a branch release-please watches. |
| `story/<id>-<slug>` | Short-lived, created fresh off `main` per story/PRD spanning multiple issues — **not** persistent, deleted immediately after it merges into `main`. Skip this layer entirely for a standalone issue that isn't part of a bigger story. | **Rebase-merge** into `main` (never squashed) — same "one changelog entry per issue, not one undifferentiated line for the whole story" reasoning as below, and never a true two-parent merge commit either (see the note below) | The PR from this branch into `main` is what triggers the rc-image build (once per completed story, not once per issue — see ADR-0010). Because the branch is deleted right after merging, it never persists alongside `main` the way the old `development` branch did — there's no second permanent branch left to accumulate stale duplicate-content commits, which is what caused the divergence problem ADR-0010 fixes. |
| `main` | release-please's target; a merge here is a real release event | — | Tags and GitHub Releases are only ever cut from here. |

**Why not a true (two-parent) merge commit.** A true merge preserves each incoming commit's original date — its manifest releaser walks `main`'s commit history newest-first *by commit date* and stops as soon as it reaches the previous release's commit SHA (`src/manifest.ts`, the `releaseCommitsFound >= expectedShas` break). If a `story/*` or `feature/*` branch sat unmerged for a while, its commits could be chronologically *older* than `main`'s own last release commit (release-please's own auto-generated `chore(main): release X.Y.Z` commit), and the walk would stop before ever reaching them — silently excluding them from the next changelog and version bump, permanently, since no later commit changes their dates. This was discovered empirically in the old two-branch model (see the commit graph around `70a4d97`/`b06443b`: `#48`/`#49`/`#52`/`#53`'s commits predated `b06443b` and were never picked up) and the same risk applies to any branch merged into `main` here. Rebase-merging avoids this because rebasing rewrites each commit's date to "now."
...
- **No floating rc-channel mechanism in OSS GoReleaser.** `--snapshot` deliberately never publishes anything (`--skip=announce,publish,validate` is implied). `--nightly` — the actual rolling/floating-tag feature — is GoReleaser Pro-only. Since a free/OSS pipeline for the rc-image channel would need a separate `docker/build-push-action`-based step regardless, routing the tagged-release channel through GoReleaser and the rc channel around it would mean two different build mechanisms for the same Dockerfiles — so both channels use the same plain `docker/build-push-action` mechanism instead, and GoReleaser is not used anywhere in the pipeline.

### Docker images

Both binaries are built as separate multi-stage images (`Dockerfile.blog`, `Dockerfile.migrate`): a `golang:1.26` build stage running natively on the build platform (`--platform=$BUILDPLATFORM`) cross-compiling via `GOOS`/`GOARCH` (no QEMU emulation), producing `linux/amd64` + `linux/arm64` images from one build; a `gcr.io/distroless/static-debian12` final stage (no shell, minimal attack surface, includes the CA cert bundle Neon's `sslmode=require` needs). `.dockerignore` excludes only `bin/` (gitignored build output, irrelevant to the build stage) — deliberately not `docs/` or `.devcontainer/`, since both are git-tracked and excluding them made every build look dirty to Go's VCS stamping regardless of actual working-tree state (see `docs/adr/0007-fix-vcs-version-to-report-tags.md`); `.git` itself is never excluded either, since the build stage's VCS version stamping depends on it being present. Both images publish to GHCR (`ghcr.io/jonathanschwarzhaupt/home-blog`, `ghcr.io/jonathanschwarzhaupt/home-blog-migrate`) via the default `GITHUB_TOKEN` (`packages: write`), no PAT needed for this specific purpose.
...
- **RC (release candidate) channel** (`dev-image.yml`, triggered when a PR into `main` — from a `story/*` or standalone `feature/*` branch — is opened or gets a new push while open): computes the next version via `release-please release-pr --dry-run` against `main`'s HEAD — read-only against GitHub's API, creates no PR — and tags the image `X.Y.Z-rc.<run_number>.<short-sha>` (a valid SemVer 2.0 prerelease identifier, and a valid Docker tag, since Docker tags can't contain `+`) plus a floating `:rc`. It also pushes a real git tag, `vX.Y.Z-rc.<run_number>.<short-sha>`, at the exact commit built, so `internal/vcs.Version()` reports that string instead of a bare commit SHA. This tag is never wrapped in an actual GitHub Release, which is what keeps it safe from release-please's own bookkeeping: traced `latestReleaseVersion()` in release-please's source, and it resolves "the last release" from (1) merged PRs matching release-please's own internal branch-naming pattern, then (2) real GitHub Release objects, only falling back to (3) raw tag scanning if neither of the first two finds anything. This repo already has real releases, so step 2 always resolves and a plain rc tag is never even considered — see `docs/adr/0010-retire-development-for-trunk-based-release-please.md` for the full argument (this supersedes ADR-0008's original safety argument, which relied on a rebase-erasure mechanism that no longer applies once there's no `development` branch to erase commits from).

### Neon branch topology

Three distinct branches of the same Neon project, not three different databases: `production` (the one real database, fed only on a tagged release — see ADR-0002/ADR-0003 for which deployment that is), a persistent `development` branch isolated from it (feeding the always-on admin-preview environment the homelab repo runs — this Neon branch's name is independent of the git branch of the same name retired in `docs/adr/0010-retire-development-for-trunk-based-release-please.md`; the database branch still exists, only the git branch was retired), and ephemeral `preview/pr-<number>` branches (created by `migration-check.yml` per open PR touching migration-relevant paths, deleted on success, left alive on failure for debugging). `docs/neon-branches.md` is the source of truth for this topology and where each branch's DSN lives — this section only records the two facts specific to the CI pipeline itself:
